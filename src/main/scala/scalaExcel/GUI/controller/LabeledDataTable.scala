...
class LabeledDataTable(_dataWindow: DataWindow,
  ...
  private def contentsToCells(filter: (((Int, Int), (Int, Int), String, Any)) => Boolean) =
    translatedContents
      .filter(filter)
      .foldLeft(Map[(Int, Int), SheetCell]())((cells, content) =>
      cells + (content._1 -> SheetCell.newEvaluated(content._2, content._3, content._4)))

  val data = {
    // transform cell contents contained in window into SheetCells
...
object LabeledDataTable {
  ...
  def buildDataTable(rows: Int, columns: Int, data: Map[(Int, Int), SheetCell], dataWindow: DataWindow): DataTable = {
    new DataTable() ++=
      List.range(0, rows).map(i => new DataRow() ++=
        List.range(0, columns).map(j =>
          ObjectProperty.apply(data.getOrElse((i, j), SheetCell.newEmpty(dataWindow.windowToAbsolute((i, j)))))))
  }
